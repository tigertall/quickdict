...
use crate::engine::types::{DictKind, DictInfo, ArticleData, Dictionary};
...
impl DictManager {
    pub fn new() -> Self {
        Self { dicts: Vec::new(), active: Vec::new(), mdict_paths_index: Vec::new() }
    }

    // ========== 统一接口 ==========

    pub fn all(&self) -> &[Arc<dyn Dictionary>] { &self.dicts }
    ...
    pub fn toggle_dict(&mut self, id: usize, enabled: bool) {
        if id < self.active.len() { self.active[id] = enabled; }
    }
    ...
    pub fn dict_infos(&self) -> Vec<DictInfo> {
        self.dicts.iter().enumerate()
            ...
            path: self.mdict_paths_index.get(i).and_then(|p| p.clone()).unwrap_or_default(),
    ...
    pub fn try_online(&self, word: &str) -> Option<ArticleData> {
        for d in self.enabled() {
            if d.kind().is_online() {
                if let Some(a) = d.lookup_exact(word) { return Some(a); }
            }
        }
        None
    }
    ...
    pub fn active_stardict(&self) -> Vec<&DictionaryReader> {
        self.dicts.iter().enumerate()
            ...
            .filter_map(|(_, d)| d.as_any().downcast_ref::<StarDictDict>().map(|sd| sd.inner.as_ref()))
            .collect()
    }

    pub fn active_dict_names(&self) -> Vec<String> {
        self.active_stardict().iter().map(|d| d.info.bookname.clone()).collect()
    }

    // ========== MDX ==========

    pub fn mdict_lookup(&self, word: &str) -> Option<ArticleData> {
        for (i, d) in self.dicts.iter().enumerate() {
            if d.kind() == DictKind::Mdx && self.active.get(i).copied().unwrap_or(true) {
                if let Some(a) = d.lookup_exact(word) { return Some(a); }
            }
        }
        None
    ...
    pub fn swap_dicts(&mut self, i: usize, j: usize) {
        if i >= self.dicts.len() || j >= self.dicts.len() { return; }
        self.dicts.swap(i, j);
    ...
    pub fn move_dict_up(&mut self, idx: usize) {
        if idx > 0 { self.swap_dicts(idx, idx - 1); }
    }

    /// 下移词典
    pub fn move_dict_down(&mut self, idx: usize) {
        if idx + 1 < self.dicts.len() { self.swap_dicts(idx, idx + 1); }
    }

    /// 导出当前词典顺序（用于持久化）
    pub fn export_order(&self) -> Vec<String> {
        self.dicts.iter().map(|d| {
            match d.kind() {
    ...
    pub fn reorder_by(&mut self, order: &[String]) {
        ...
        let mut indexed: Vec<(usize, usize)> = self.dicts.iter().enumerate()
            .map(|(i, d)| {
    ...
    pub fn online_idx(&self, service_id: &str) -> Option<usize> {
        self.dicts.iter().position(|d| d.kind().online_id() == Some(service_id))
    }
    ...
    pub fn import_active_states(&mut self, states: &[(String, bool)]) {
        for (i, d) in self.dicts.iter().enumerate() {
            ...
            if let Some(&(_, enabled)) = states.iter().find(|(k, _)| k == &key) {
                if i < self.active.len() { self.active[i] = enabled; }
            }
        }
    }
    ...
    pub fn load_from_cache(&mut self, scanned: &[(String, String, String)]) -> usize {
        ...
        for (kind, name, path) in scanned {
            ...
            if self.dicts.iter().any(|d| d.name() == name.as_str()) { continue; }
            match kind.as_str() {
                "Mdx" => {
                    match MdxReader::open(p) {
                        Ok(reader) => {
                            let r: Arc<dyn Dictionary> = Arc::new(MdxDict { inner: Arc::new(reader) });
                            let rc = r.clone();
                            self.dicts.push(r);
                            self.active.push(true);
                            ...
                            if fp.is_file() && fp.extension().map(|x| x == "ifo").unwrap_or(false) {
                                if let Ok(r) = DictionaryReader::open(&fp) {
                                    if r.info.bookname == name.as_str() {
                                        self.dicts.push(Arc::new(StarDictDict { inner: Arc::new(r) }));
                                        self.active.push(true);
                                        self.mdict_paths_index.push(Some(path.clone()));
                                        count += 1;
                                    }
    ...
    pub fn scan_directories(&self, dirs: &[PathBuf]) -> Result<Vec<(String, String, String)>, String> {
        let mut found = Vec::new();
        for dir in dirs { self.scan_dir_rec(dir, &mut found)?; }
        Ok(found)
    }

    fn scan_dir_rec(&self, dir: &PathBuf, found: &mut Vec<(String, String, String)>) -> Result<(), String> {
        if let Ok(entries) = std::fs::read_dir(dir) {
            for e in entries.flatten() {
                let p = e.path();
                if p.is_file() && p.extension().map(|x| x == "ifo").unwrap_or(false) {
                    if let Ok(r) = DictionaryReader::open(&p) {
                        let path_str = p.parent().map(|x| x.to_string_lossy().to_string()).unwrap_or_default();
                ...
                } else if p.is_dir() { self.scan_dir_rec(&p, found)?; }
            }
        }
        Ok(())
    }
    ...
    /// 向后兼容：search_engine 使用
    pub fn active_dictionaries(&self) -> Vec<&DictionaryReader> { self.active_stardict() }
...
pub struct StarDictDict { pub inner: Arc<DictionaryReader> }

impl Dictionary for StarDictDict {
    fn name(&self) -> &str { &self.inner.info.bookname }
    fn word_count(&self) -> usize { self.inner.word_count() }
    fn kind(&self) -> DictKind { DictKind::StarDict }
    fn lookup_exact(&self, w: &str) -> Option<ArticleData> { self.inner.lookup_exact(w) }
    fn as_any(&self) -> &dyn std::any::Any { self }
}

pub struct MdxDict { pub inner: Arc<MdxReader> }

impl Dictionary for MdxDict {
    fn name(&self) -> &str { self.inner.name() }
    fn word_count(&self) -> usize { self.inner.word_count() }
    fn kind(&self) -> DictKind { DictKind::Mdx }
    fn lookup_exact(&self, w: &str) -> Option<ArticleData> { self.inner.lookup_exact(w) }
    fn as_any(&self) -> &dyn std::any::Any { self }
}
...
impl Dictionary for BaiduDict {
    fn name(&self) -> &str { "Baidu Translate" }
    fn word_count(&self) -> usize { 0 }
    fn kind(&self) -> DictKind { DictKind::Online("baidu".into()) }
    fn lookup_exact(&self, w: &str) -> Option<ArticleData> {
        ...
        if appid.is_empty() || apikey.is_empty() { return None; }
        BaiduTranslateClient::new(&appid, &apikey).translate(w, "zh")
    }
    fn as_any(&self) -> &dyn std::any::Any { self }
}
